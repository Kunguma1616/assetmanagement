...
import { AlertTriangle, TrendingUp, PoundSterling, Truck, AlertCircle, Loader, Search, ChevronDown, ChevronUp, TrendingDown, MessageCircle } from 'lucide-react';
...
export default function ServiceMaintenanceCosts() {
  ...
  const startServiceCostsTour = () => {
    const steps = [
      {
        element: '#cost-summary-cards',
        popover: {
          title: 'Cost Summary',
          description: 'These cards show the overall fleet cost, service cost, maintenance cost, and average cost per vehicle.',
          side: 'bottom' as const,
          align: 'start' as const,
        },
      },
      {
        element: '#hsbc-leases-section',
        popover: {
          title: 'HSBC Leases',
          description: 'This section helps you review lease records, matched vehicles, and finance-related asset information.',
          side: 'top' as const,
          align: 'start' as const,
        },
      },
      {
        element: '#cost-insights',
        popover: {
          title: 'Cost Insights',
          description: 'Here you can quickly identify the vehicles with the highest maintenance and service costs.',
          side: 'top' as const,
          align: 'start' as const,
        },
      },
      {
        element: '#cost-filters',
        popover: {
          title: 'Search and Filters',
          description: 'Use these filters to narrow results by vehicle, trade group, and cost type so you can focus on the right records.',
          side: 'top' as const,
          align: 'start' as const,
        },
      },
      {
        popover: {
          title: 'You Are Ready',
          description: 'You can now review service and maintenance cost trends, spot expensive vehicles, and take action where needed.',
          side: 'over' as const,
      ...
      <SectionHero
        title={
          <div className="flex items-center gap-2 flex-wrap">
            <span>Chumley Service &amp; Maintenance Costs</span>
            {isRestricted && (
              <span
                style={{
                  padding: '4px 8px',
        ...
        subtitle={isRestricted ? `Viewing vehicles for ${userTrade}` : 'Vehicle fleet analysis'}
        onBack={() => navigate(-1)}
        actions={<PageHeaderActions onStartGuide={startServiceCostsTour} />}
      />

      <div className="max-w-7xl mx-auto px-4 py-8 pt-0">

        {/* Summary Cards */}
        <div id="cost-summary-cards" className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
      <div className="bg-white rounded-lg p-6 shadow-sm border" style={{ borderColor: colors.grayscale.border }}>
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold mt-2" style={{ color: colors.primary.darker }}>
        ...
        {/* HSBC Leases Section */}
        <div id="hsbc-leases-section" className="bg-white rounded-lg p-6 mb-8 shadow-sm border" style={{ borderColor: colors.grayscale.border }}>
          {/* Header row */}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
            <div style={{ padding: '10px 14px', backgroundColor: colors.primary.subtle, borderRadius: '6px' }}>
              ...
              <p style={{ fontSize: '20px', fontWeight: 700, color: '#17325E' }}>{filteredCsvLeases.length}</p>
            </div>
            <div style={{ padding: '10px 14px', backgroundColor: '#EFF6FF', borderRadius: '6px' }}>
              ...
              <p style={{ fontSize: '20px', fontWeight: 700, color: '#17325E' }}>{filteredCsvLeases.filter(l => l['Type'] === 'Motor Vehicle').length}</p>
            </div>
            <div style={{ padding: '10px 14px', backgroundColor: '#FFF7ED', borderRadius: '6px' }}>
              ...
              <p style={{ fontSize: '20px', fontWeight: 700, color: '#17325E' }}>{filteredCsvLeases.filter(l => l['Type'] === 'Equipment').length}</p>
            </div>
            <div style={{ padding: '10px 14px', backgroundColor: '#F0FDF4', borderRadius: '6px' }}>
              ...
              <p style={{ fontSize: '20px', fontWeight: 700, color: '#17325E' }}>{filteredCsvLeases.filter(l => l['_van_number']).length}</p>
            </div>
          </div>
        ...
        {/* Cost Insights */}
        <div id="cost-insights" className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
        ...
        {/* Filters & Search */}
        <div id="cost-filters" className="bg-white rounded-lg p-6 mb-8 shadow-sm border" style={{ borderColor: colors.grayscale.border }}>
