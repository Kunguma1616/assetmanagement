...
import {
  ...
  ChevronLeft, ChevronRight, Filter, X, Download,
  TrendingUp, Package, Clock, Users, BarChart2,
} from 'lucide-react';
import {
...
const COLORS = [C.blue, C.blueLight, C.green, C.orange, C.red, C.blueDark, C.gray, C.graySubtle];
...
function StatCard({ label, value, sub, accent, icon: Icon }: {
  ...
  return (
    <div style={{
      ...
      <p style={{ fontSize: 11, color: C.gray, margin: "0 0 4px", fontWeight: 700,
        textTransform: "uppercase", letterSpacing: "0.07em", fontFamily: FONT }}>
        {label}
      </p>
      <p style={{ fontSize: 30, fontWeight: 900, color: C.title, margin: "0 0 2px",
...
function ValChip({ val, green = false }: { val: string | null; green?: boolean }) {
  if (!val) return (
    <span style={{ color: C.border, fontStyle: "italic", fontSize: 11, fontFamily: FONT }}>unassigned</span>
  );
  if (isRawId(val)) return (
    <span style={{
      padding: "2px 8px", borderRadius: 6, fontSize: 10, fontFamily: "monospace",
      background: C.borderSubtle, color: C.gray, border: `1px solid ${C.border}`,
    }}>
    ...
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: C.bg, gap: 12 }}>
      <div style={{ background: C.white, borderRadius: 16, padding: 40, textAlign: "center", boxShadow: "0 4px 24px rgba(0,0,0,0.08)" }}>
        ...
        <button onClick={load} style={{
          ...
          padding: "10px 24px", cursor: "pointer", fontFamily: FONT, fontWeight: 700, fontSize: 13,
        }}>Retry</button>
      </div>
    </div>
  );
    ...
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: FONT }}>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .btn-sec:hover { background: ${C.borderSubtle} !important; }
        input:focus, select:focus { outline: none; border-color: ${C.blue} !important; box-shadow: 0 0 0 3px ${C.blueBorder}; }
      `}</style>

      {/* ── HEADER ───────────────────────────────────────────────────────────── */}
      <div style={{
        background: `linear-gradient(135deg, ${C.blueDark} 0%, ${C.blue} 55%, ${C.blueLight} 100%)`,
        padding: "28px 36px 80px", position: "relative", overflow: "hidden",
      }}>
        <div style={{ position: "absolute", top: -40, right: -40, width: 200, height: 200, borderRadius: "50%", background: "rgba(255,255,255,0.05)" }} />
        <div style={{ position: "absolute", bottom: -50, right: 200, width: 140, height: 140, borderRadius: "50%", background: "rgba(241,255,36,0.08)" }} />
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", position: "relative" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            <div style={{
              ...
              <ArrowRightLeft style={{ width: 26, height: 26, color: C.yellow }} />
            </div>
            <div>
              <p style={{ fontSize: 11, color: "rgba(255,255,255,0.6)", margin: "0 0 2px", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em" }}>
                Asset Management
              </p>
              <h1 style={{ fontSize: 26, fontWeight: 900, color: C.white, margin: 0, letterSpacing: "-0.02em" }}>
                Allocation History
              </h1>
              <p style={{ fontSize: 12, color: "rgba(255,255,255,0.55)", margin: "4px 0 0", fontWeight: 500 }}>
          ...
          <button onClick={load} style={{
            ...
            fontFamily: FONT, backdropFilter: "blur(8px)", transition: "opacity 0.15s",
          }}>
      ...
      <div style={{ padding: "24px 36px 40px" }}>
              ...
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={topAssets} layout="vertical" barSize={14}>
                  ...
                  <YAxis type="category" dataKey="name" width={110} axisLine={false} tickLine={false}
                    tick={{ fontSize: 10, fontFamily: FONT, fill: C.body, fontWeight: 600 }}
        ...
        <div style={{ background: C.white, borderRadius: 16, overflow: "hidden", boxShadow: "0 2px 12px rgba(39,84,157,0.07)", border: `1px solid ${C.borderSubtle}` }}>
          ...
          <div style={{ padding: "18px 24px", borderBottom: `1px solid ${C.borderSubtle}` }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                ...
                <div>
                  <h2 style={{ fontSize: 15, fontWeight: 800, color: C.title, margin: 0, fontFamily: FONT }}>
                    Full Allocation Log{" "}
                    <span style={{ color: C.blue }}>({fmt(filtered.length)})</span>
                  </h2>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                ...
                <div style={{ position: "relative" }}>
                  ...
                  <input
                    ...
                    style={{
                      ...
                      fontFamily: FONT, width: 180, color: C.body, background: C.white,
                    }}
                  />
                </div>
                ...
                <button onClick={() => setShowFilters(v => !v)} style={{
                  ...
                  padding: "9px 14px", borderRadius: 9, border: `1px solid ${showFilters || activeFilters > 0 ? C.blue : C.border}`,
                  background: showFilters || activeFilters > 0 ? C.blue : C.white,
                  color: showFilters || activeFilters > 0 ? C.white : C.body,
                  cursor: "pointer", fontSize: 12, fontWeight: 700, fontFamily: FONT, transition: "all 0.15s",
                }}>
                  <Filter style={{ width: 13, height: 13 }} /> Filters
                  {activeFilters > 0 && (
                    <span style={{ background: "rgba(255,255,255,0.25)", borderRadius: "50%", width: 18, height: 18, fontSize: 10, fontWeight: 900, display: "flex", alignItems: "center", justifyContent: "center" }}>
                      {activeFilters}
                    </span>
                  )}
                </button>
                ...
                {activeFilters > 0 && (
                  <button onClick={clearFilters} style={{
                    display: "flex", alignItems: "center", gap: 6,
                    padding: "9px 14px", border: `1px solid ${C.red}`, borderRadius: 9,
                ...
                <button onClick={() => exportCSV(filtered)} style={{
                  display: "flex", alignItems: "center", gap: 6,
                  padding: "9px 14px", border: "none", borderRadius: 9,
            ...
            {showFilters && (
              <div style={{
                marginTop: 14, background: C.blueSubtle, borderRadius: 10,
                padding: 16, border: `1px solid ${C.blueBorder}`,
                ...
                {[
                  ...
                  <div key={label}>
                    ...
                    <select value={value} onChange={e => onChange(e.target.value)} style={{
                      ...
                      borderRadius: 8, fontSize: 12, fontFamily: FONT, background: C.white, color: C.body,
                    }}>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              <thead>
                <tr style={{ background: C.blueSubtle }}>
                  {["#", "Asset Name", "From", "To (Assigned)", "Date", "Time"].map((h, i) => (
                    <th key={h} style={{
                      textAlign: "left", padding: "11px 20px",
                      fontWeight: 800, fontSize: 10, color: C.blueDark,
                      borderBottom: `1px solid ${C.blueBorder}`,
                      fontFamily: FONT, textTransform: "uppercase", letterSpacing: "0.07em",
                      whiteSpace: "nowrap",
                    }}>
                      {h}
                    </th>
                ...
                {paginated.length === 0 ? (
                    ...
                    <td colSpan={6}>
                      ...
                      {activeFilters > 0 && (
                        <div style={{ textAlign: "center", paddingBottom: 24 }}>
                          <button onClick={clearFilters} style={{ color: C.blue, fontSize: 12, fontWeight: 700, fontFamily: FONT, background: "none", border: "none", cursor: "pointer", textDecoration: "underline" }}>
                            Clear all filters
                          </button>
                        </div>
                      )}
                  ...
                  return (
                    <tr key={`${a.asset_id}-${i}`} className="trow" style={{ borderBottom: `1px solid ${C.borderSubtle}`, transition: "background 0.12s" }}>
                      <td style={{ padding: "12px 20px", color: C.gray, fontWeight: 700, fontSize: 11 }}>
                      ...
                      <td style={{ padding: "12px 20px" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                          ...
                          <span style={{ fontWeight: 700, color: C.title, fontSize: 13 }}>
                            {a.asset_name ?? '—'}
                          </span>
                        </div>
                      </td>
                      ...
                      <td style={{ padding: "12px 20px", fontSize: 11, color: C.gray, fontFamily: "monospace", whiteSpace: "nowrap" }}>
                        {dt.time}
                      </td>
                    </tr>
                  );
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                {[
                  ...
                  <button key={label} onClick={action} disabled={disabled} style={{
                    ...
                    background: C.white, color: disabled ? C.border : C.body,
                    cursor: disabled ? "not-allowed" : "pointer", fontWeight: 700, fontSize: 12, fontFamily: FONT,
                  }}>{label}</button>
                ))}
                {(() => {
                  const range: number[] = [];
                  ...
                  return range.map(p => (
                    <button key={p} onClick={() => setPage(p)} style={{
                      width: 32, height: 32, borderRadius: 7, border: `1px solid ${p === page ? C.blue : C.border}`,
                      background: p === page ? C.blue : C.white,
                      color: p === page ? C.white : C.body,
                      cursor: "pointer", fontWeight: 800, fontSize: 12, fontFamily: FONT,
                    }}>{p}</button>
                  ));
                })()}
                {[
                ...
                <span style={{ fontSize: 11, color: C.gray, fontFamily: FONT, marginLeft: 4 }}>
                  {page}/{totalPages}
                </span>
              </div>
            </div>
...
function NoData({ message = "No data available", tall }: { message?: string; tall?: boolean }) {
  return (
    <div style={{
      height: tall ? 160 : 140, display: "flex", flexDirection: "column",
