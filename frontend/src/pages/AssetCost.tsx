...
import { DollarSign, Search, Download, RefreshCw, Filter, TrendingUp, Package, BarChart2, ArrowUpRight, ArrowDownRight } from "lucide-react";
import {
  ...
  return (
    <div style={{
      ...
      <div>
        ...
        <p style={{ fontSize: 28, fontWeight: 900, color: C.title, margin: "0 0 2px",
  ...
  const loadCostData = async () => {
    try {
      ...
      if (typeRes.ok) {
        ...
        if (t.success && Array.isArray(t.cost_by_type)) setCostByType(t.cost_by_type);
      }
    } catch (err: any) {
    ...
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: C.bg }}>
      <div style={{ textAlign: "center" }}>
        <div style={{
          ...
          border: `3px solid ${C.blueBorder}`, borderTopColor: C.blue,
    ...
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: C.bg }}>
      <div style={{ textAlign: "center", background: C.white, borderRadius: 16, padding: 40, boxShadow: "0 4px 24px rgba(0,0,0,0.08)" }}>
        <div style={{ width: 48, height: 48, borderRadius: 12, background: C.redSubtle, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px" }}>
          <DollarSign style={{ color: C.red, width: 24, height: 24 }} />
        </div>
        ...
        <button onClick={loadCostData} style={{
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: FONT }}>
      ...
      <div style={{
        ...
        <div style={{ position: "absolute", bottom: -60, right: 120, width: 160, height: 160, borderRadius: "50%", background: "rgba(241,255,36,0.08)" }} />
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", position: "relative" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            <div style={{
              ...
              <DollarSign style={{ width: 28, height: 28, color: C.yellow }} />
            </div>
            <div>
      ...
      <div style={{ padding: "0 36px", marginTop: -52, position: "relative", zIndex: 10 }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16 }}>
          <StatCard label="Total Spend"   value={`£${totalSpend.toLocaleString("en-GB", { maximumFractionDigits: 0 })}`} sub={`${assets.length} assets tracked`}  accent={C.blue}   icon={DollarSign}  trend={4.2} />
        ...
        {activeTab === "overview" && (
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 20, marginBottom: 20 }}>
              ...
              <div className="card-hover" style={{ background: C.white, borderRadius: 16, padding: 24, boxShadow: "0 2px 12px rgba(39,84,157,0.07)", border: `1px solid ${C.borderSubtle}` }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
                  <div>
                    <h3 style={{ fontSize: 15, fontWeight: 800, color: C.title, margin: "0 0 2px" }}>Spend by Asset Type</h3>
                    <p style={{ fontSize: 11, color: C.gray, margin: 0 }}>Total expenditure per category</p>
                  </div>
              ...
              <div className="card-hover" style={{ background: C.white, borderRadius: 16, padding: 24, boxShadow: "0 2px 12px rgba(39,84,157,0.07)", border: `1px solid ${C.borderSubtle}` }}>
                <div style={{ marginBottom: 20 }}>
                  <h3 style={{ fontSize: 15, fontWeight: 800, color: C.title, margin: "0 0 2px" }}>Cost Distribution</h3>
                  <p style={{ fontSize: 11, color: C.gray, margin: 0 }}>Share of spend by type</p>
                </div>
                {costByTypeData.length > 0 ? (
                  <>
            ...
            <div className="card-hover" style={{ background: C.white, borderRadius: 16, padding: 24, boxShadow: "0 2px 12px rgba(39,84,157,0.07)", border: `1px solid ${C.borderSubtle}` }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
                <div>
                  <h3 style={{ fontSize: 15, fontWeight: 800, color: C.title, margin: "0 0 2px" }}>Top 8 Most Expensive Assets</h3>
                  <p style={{ fontSize: 11, color: C.gray, margin: 0 }}>Ranked by purchase value</p>
                </div>
          ...
          <>
              ...
              <div style={{ position: "relative", flex: 1 }}>
                ...
                <input
                  ...
                  style={{
                    ...
                    color: C.body, boxSizing: "border-box",
            ...
            <div style={{ background: C.white, borderRadius: 16, overflow: "hidden", boxShadow: "0 2px 12px rgba(39,84,157,0.07)", border: `1px solid ${C.borderSubtle}` }}>
              <div style={{ padding: "18px 24px", borderBottom: `1px solid ${C.borderSubtle}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <h3 style={{ fontSize: 15, fontWeight: 800, color: C.title, margin: 0 }}>Asset Cost Details</h3>
                <span style={{ fontSize: 12, color: C.gray, fontFamily: FONT, fontWeight: 600 }}>
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                    <thead>
                      <tr style={{ background: C.blueSubtle }}>
                        {["#", "Asset Name", "Cost", "Type", "Status", "Purchase Date"].map((h, i) => (
                          <th key={h} style={{
                    ...
                    <tbody>
                      {filteredAssets.map((asset, i) => (
                        <tr key={asset.id || i} className="row-hover" style={{ borderBottom: `1px solid ${C.borderSubtle}`, transition: "background 0.12s" }}>
                          <td style={{ padding: "13px 20px", color: C.gray, fontWeight: 700, fontSize: 12 }}>
                            {i + 1}
                          </td>
                          <td style={{ padding: "13px 20px", fontWeight: 700, color: C.title }}>
                            {asset.name}
                          </td>
                          <td style={{ padding: "13px 20px", textAlign: "right" }}>
                            <span style={{ fontWeight: 900, color: C.blue, fontSize: 14 }}>
                          ...
                          <td style={{ padding: "13px 20px" }}>
                            <span style={{
                              ...
                              border: `1px solid ${C.blueBorder}`, borderRadius: 20,
                          ...
                          <td style={{ padding: "13px 20px", color: C.graySubtle, fontWeight: 600 }}>
                            {asset.purchase_date
...
function NoData({ message = "No data available" }: { message?: string }) {
  return (
    <div style={{
      ...
      <div style={{ width: 40, height: 40, borderRadius: 10, background: "#F7F9FD", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <BarChart2 style={{ width: 20, height: 20, color: "#CDD1DA" }} />
      </div>
      <p style={{ color: "#848EA3", fontSize: 13, fontFamily: "'Montserrat',sans-serif", fontWeight: 600, margin: 0 }}>
        {message}
      </p>
    </div>
  );
