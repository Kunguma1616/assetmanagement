...
const FinanceApprovals: React.FC = () => {
  ...
  const loadApprovals = async () => {
    try {
      setLoading(true);
      const res = await fetch(`${API_BASE}/finance/pending`);
  ...
  const handleApproval = async (approve: boolean) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/finance/approve/${selectedRecord.id}`, {
        method: "POST",
    ...
    <div style={{ fontFamily: FONT, background: C.gray.surface, minHeight: "100vh", padding: "40px 20px" }}>
      ...
      <div style={{ maxWidth: "1200px", margin: "0 auto 40px" }}>
        ...
        <p style={{ fontSize: "14px", color: C.gray.subtle, margin: 0 }}>
          {approvals.length} request{approvals.length !== 1 ? "s" : ""} pending finance review
        </p>
      </div>
        ...
        <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
          {approvals.map((approval) => (
            ...
            >
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "16px", fontSize: "12px", color: C.gray.caption }}>
                <div>
                ...
                <div>
                  <p style={{ margin: "0 0 4px 0" }}>Director Approved</p>
                  <p style={{ margin: 0, fontWeight: 600, color: C.success.default }}>
                    {approval.directorApprovedBy}
                  </p>
                </div>
              </div>
            </div>
        ...
        >
          ...
          >
            ...
            <div style={{ background: C.gray.negative, padding: "12px", borderRadius: "8px", marginBottom: "24px", fontSize: "12px", color: C.gray.subtle }}>
              ...
              <p style={{ margin: "0 0 4px 0" }}>✓ Director: {selectedRecord.directorApprovedBy}</p>
        ...
        >
          ...
          >
            ...
            <p style={{ fontSize: "13px", color: C.gray.body, margin: "0 0 20px 0" }}>
              {action === "approve"
                ? "This will complete the approval process and process the payment."
                : "This will reject the request and send it back to the director."}
            </p>

            <div style={{ marginBottom: "20px" }}>
              <label style={{ fontSize: "12px", fontWeight: 700, color: C.gray.subtle, display: "block", marginBottom: "8px" }}>
                {action === "approve" ? "Processing Notes" : "Rejection Reason"} (optional)
              </label>
              <textarea
                value={comment}
                onChange={(e) => setComment(e.target.value)}
                placeholder={action === "approve" ? "Add any processing notes..." : "Please explain why..."}
                style={{
                  width: "100%",
                  minHeight: "80px",
                  padding: "12px",
            ...
            <div style={{ display: "flex", gap: "12px" }}>
              ...
              >
                {submitting ? "Processing..." : action === "approve" ? "Approve & Complete" : "Reject"}
              </button>
            </div>
          </div>
        </div>
