...
Changes from original:
  ...
  - Added analyse_vehicle_images for VCR AI image analysis (llama-3.2-11b-vision-preview)
        ...
        try:
            ...
            if intent == 'list_webfleet_drivers':
                ...
                if entity and self.driver_cache:
                    ...
                    if matches:
                        return matches
                    return [{"status": "no_match", "message": f"No drivers found matching '{entity}' in cached data"}]
                elif not self.driver_cache:
                    return [{"status": "no_cache", "message": "Driver cache not loaded. Please ensure app has started properly."}]
                return self._webfleet_not_available()

            elif intent == 'get_drivers_by_score':
                if self.driver_cache:
                    score_threshold = params.get('score', 8.0)
                    matches = [d for d in self.driver_cache if d.get('driving_score', 0) >= score_threshold]
                    if matches:
                        matches.sort(key=lambda x: -x.get('driving_score', 0))
                        return matches
                    return [{"status": "no_match", "message": f"No drivers found with score >= {score_threshold}"}]
                return [{"status": "no_cache", "message": "Driver cache not loaded. Please ensure app has started properly."}]
            ...
            # ── SALESFORCE INTENTS ───────────────────────────
                ...
                return self.sf.get_all_vehicles()
...
Keep response under 200 words."""
    ...
    # ─────────────────────────────────────────────────────────
    # VCR AI IMAGE ANALYSIS
    # ─────────────────────────────────────────────────────────

    def analyse_vehicle_images(
        self,
        images: List[Dict],       # [{ "title": "...", "base64": "...", "content_type": "image/jpeg" }]
        engineer_name: str,
        vehicle_name: str,
        reg_no: str
    ) -> List[Dict]:
        """
        Sends each VCR image to Llama Vision and returns a condition report per image.
        Uses the SAME self.client (same API key), different model: llama-3.2-11b-vision-preview
        """
        if not self.is_available():
        ...
        for image in images:
            raw = ""
            try:
                prompt = f"""You are a professional fleet vehicle inspector AI working for a UK utilities company.

Engineer who submitted this report: {engineer_name}
Vehicle: {vehicle_name} | Registration: {reg_no}
Image title: {image.get('title', 'Unknown')}

Carefully analyse this vehicle image and return ONLY a valid JSON object. No extra text.

{{
    "engineer": "{engineer_name}",
    "vehicle": "{vehicle_name}",
    "reg_no": "{reg_no}",
    "image_title": "{image.get('title', 'Unknown')}",
    "area_captured": "describe what part of the vehicle is shown e.g. Front Bumper, Left Side Panel, Rear Door",
    "overall_condition": "GREEN or AMBER or RED",
    "damage_detected": true or false,
    "damage_description": "describe any dents, scratches, cracks, or say None",
    "cleanliness": "Clean or Dirty or Very Dirty",
    "tyres_visible": true or false,
    "tyre_condition": "Good or Worn or Flat or Not Visible",
    "lights_visible": true or false,
    "lights_condition": "Working or Damaged or Not Visible",
    "action_required": "None or Monitor or Repair Needed or Urgent",
    "priority": "LOW or MEDIUM or HIGH or CRITICAL",
    "inspector_notes": "1-2 sentence summary for the fleet manager about what was observed"
}}

Rules:
- GREEN = no issues, vehicle looks good
- AMBER = minor issues, monitor or schedule repair
- RED = serious damage, urgent action needed
- Be specific about what you see, not generic
- If image is blurry or unclear, set overall_condition to AMBER and note it
"""

                response = self.client.chat.completions.create(
                    model="meta-llama/llama-4-scout-17b-16e-instruct", # Vision model, same API key
                    messages=[
                        {
                            "role": "user",
                            "content": [
                ...
                print(f"[VCR_AI] ✅ Analysed: {image.get('title')} → {parsed.get('overall_condition')}")

            except json.JSONDecodeError:
                print(f"[VCR_AI] ⚠️ JSON parse failed for {image.get('title')}")
                results.append({
                    "image_title": image.get('title', 'Unknown'),
                    "overall_condition": "AMBER",
                ...
                print(f"[VCR_AI] ❌ Error analysing {image.get('title')}: {e}")
                results.append({
                    "image_title": image.get('title', 'Unknown'),
                    "overall_condition": "AMBER",
                    "error": str(e),
