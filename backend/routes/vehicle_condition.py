...
# Active allocations — your exact query
...
# Engineers SOQL — for trade lookup
ENGINEERS_SOQL = """
...
# ALL allocations (no date filter) — used as fallback to get vehicle for engineers
# who don't have a current active allocation (e.g. gap in dates)
...
def extract_van_name(alloc: dict) -> str:
    """
    Try Van_Number__c first, then Vehicle__r.Name.
    Both fields are fetched in the SOQL.
    """
...
# ─── Debug Endpoint ───────────────────────────────────────────────────────────
# Hit this first to diagnose exactly what Salesforce returns for an engineer.
# Usage: GET /api/vehicle-condition/debug/allocation/{engineer_name}
# e.g.:  /api/vehicle-condition/debug/allocation/Abdulwajed%20Nourestani%20(E4)

@router.get("/debug/allocation/{engineer_name}")
def debug_allocation(engineer_name: str):
    """
    Returns raw allocation data for a given engineer name.
    Use this to confirm what Vehicle__r fields are populated.
    """
    try:
...
#
#  FLOW:
#  1. Fetch active allocations (your exact SOQL with date filter)
#  2. Fetch ALL allocations (no date filter) as fallback
#  3. Fetch engineer trades from ServiceResource
#  4. Fetch ALL VCRs → index by Vehicle__c
#  5. For each engineer:
#     a. Check active allocations first
#     b. If not found → check all allocations (most recent) as fallback
#     c. Look up VCR by vehicle ID
#
...
def get_compliance_dashboard_all_allocated():
    try:
        ...
        # engineer_name → { vehicleId, vanName, regNo, active: True }
        ...
        # Engineers showing N/A have no active allocation matching the date filter.
        # We still want to show their most recent vehicle so fetch all allocations.
        ...
        for alloc in all_alloc_records:
            ...
            if not eng or not vid:
                continue
            if eng not in fallback_alloc:  # keep latest (DESC sort)
                fallback_alloc[eng] = {
                    "vehicleId": vid,
                    "vanName":   extract_van_name(alloc),
                    "regNo":     (alloc.get("Vehicle__r") or {}).get("Reg_No__c") or "N/A",
        ...
        for r in trade_records:
            ...
            if name and cat != 'EXCLUDED':
                trade_map[name] = {"category": cat, "raw": raw}

        for override in MANUAL_ENGINEERS:
            name = (override.get("Name") or "").strip()
            if name:
                raw = override.get("Trade_Lookup__c", "")
                trade_map[name] = {"category": map_trade_to_category(raw), "raw": raw}

        print(f"[VCR_DASHBOARD] Trade map: {len(trade_map)} engineers")

        # ── Step 4: Build engineer list ────────────────────────────────────────
        # Union of: engineers with active alloc + engineers with any alloc
        # Everyone in fallback_alloc who has a trade is included.
        ...
        for eng_name in all_engineer_names:
            ...
            if not trade_entry:
                ...
                for k, v in trade_map.items():
                    if k.lower() == lower:
                        ...
                        break
            if not trade_entry:
                continue  # not an active engineer we track
            ...
            # Get vehicle — prefer active allocation, fall back to most recent
            ...
            if vcr:
                ...
                if vcr_date:
                    ...
                    if days_since <= 14:
                        ...
                        print(f"[VCR_DASHBOARD] ✓ {eng_name} | {van_name} | {days_since}d ago")
                    else:
                        not_submitted_list.append({**record, "status": "Overdue"})
                        print(f"[VCR_DASHBOARD] ⚠ {eng_name} | {van_name} | {days_since}d OVERDUE")
                else:
                    not_submitted_list.append({**base_record, "latestVcrDate": None, "daysSince": None, "status": "Missing"})
            else:
                print(f"[VCR_DASHBOARD] ✗ {eng_name} | Van: {van_name} | vehicleId: {vehicle_id} → no VCR")
                not_submitted_list.append({**base_record, "latestVcrDate": None, "daysSince": None, "status": "Missing"})
...
def get_engineers_with_trades():
    try:
        ...
        for r in results:
            ...
            engineers.append({"name": eng_name, "trade": mapped, "rawTrade": raw_trade})
        ...
        for override in MANUAL_ENGINEERS:
            ...
            override_entry = {"name": override_name, "trade": map_trade_to_category(raw_trade), "rawTrade": raw_trade}
            replaced = False
