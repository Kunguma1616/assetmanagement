...
# Firebase helpers (photos stored in Firebase Storage)
try:
    from routes.firebase_service import (
        get_latest_vcr_by_van,
        get_all_vcrs_from_firebase,
    )
    FIREBASE_AVAILABLE = True
except Exception as _fb_import_err:
    print(f"[WARN] Firebase service unavailable: {_fb_import_err}")
...
EXCLUDED_TRADES = {'Vent Hygiene', 'vent hygiene', 'Key', 'key',
...
MANUAL_ENGINEERS = [
    ...
    # Electricity North - Keep only ONE instance per engineer
...
# ── Manual allocations with proper van numbers ─────────────────────────────────
MANUAL_ALLOCATIONS = [
...
# ── UPDATED: now fetches Trade_Group_Postcode__c ──────────────────────────────
ENGINEERS_SOQL = """
...
def get_compliance_dashboard_all_allocated():
    try:
        ...
        print(f"[VCR_DASHBOARD] Engineers with ACTIVE allocation: {len(alloc_by_engineer)}")

        # ── Step 2: All allocations fallback ──────────────────────────────────
        all_alloc_records = sf_service.execute_soql(ALL_ALLOCATIONS_SOQL) or []
        print(f"[VCR_DASHBOARD] All allocation rows (fallback): {len(all_alloc_records)}")
        ...
        # ── Step 3: Engineer trades ────────────────────────────────────────────
        # FIX: Now reads Trade_Group_Postcode__c and stores as locationGroup
        trade_records = sf_service.execute_soql(ENGINEERS_SOQL) or []
        trade_map: dict = {}
        for r in trade_records:
            name    = (r.get("Name") or "").strip()
            raw     = r.get("Trade_Lookup__c", "")
            # ── THE FIX: read location group from Salesforce ──────────────────
            loc_grp = (r.get("Trade_Group_Postcode__c") or "").strip()
            # e.g. "Electrical N", "Electrical S", "Drainage NW", "Plumbing E"
            # ─────────────────────────────────────────────────────────────────
            ...
            if name and cat != 'EXCLUDED':
                trade_map[name] = {
                    "category":      cat,
                    "raw":           raw,
                    "locationGroup": loc_grp,   # ← NOW POPULATED
                }
        ...
        print(f"[VCR_DASHBOARD] Trade map: {len(trade_map)} engineers")

        # ── Step 4: Build engineer list ────────────────────────────────────────
        all_engineer_names: set = set(fallback_alloc.keys()) | set(alloc_by_engineer.keys())
        # ── Include all engineers from trade_map (includes MANUAL_ENGINEERS) ────
        all_engineer_names.update(trade_map.keys())

        # ── Step 5: VCRs indexed by Vehicle__c ────────────────────────────────
        ...
        print(f"[VCR_DASHBOARD] VCRs indexed for {len(latest_vcr_by_vehicle)} vehicles")

        # ── Step 6: Classify each engineer ────────────────────────────────────
        submitted_list:     list = []
        ...
        for eng_name in all_engineer_names:
            ...
            # ── THE FIX: pull locationGroup and send it in every record ───────
            location_grp  = trade_entry.get("locationGroup", "")
            # e.g. "Electrical N" → frontend N button filters to this value
            # ─────────────────────────────────────────────────────────────────
            ...
            base_record = {
                ...
                "trade":         mapped_trade,
                "rawTrade":      raw_trade,
                "locationGroup": location_grp,  # ← ADDED: e.g. "Electrical N"
                "vanName":       van_name,
                "regNo":         reg_no,
            ...
            if vcr:
                ...
                if vcr_date:
                    ...
                    record = {**base_record, "latestVcrDate": vcr_date.date().isoformat(), "daysSince": days_since}
                    if days_since <= 14:
                        submitted_list.append({**record, "status": "Submitted"})
                        print(f"[VCR_DASHBOARD] ✓ {eng_name} | {van_name} | loc:{location_grp} | {days_since}d ago")
                    else:
                        not_submitted_list.append({**record, "status": "Overdue"})
                        print(f"[VCR_DASHBOARD] ⚠ {eng_name} | {van_name} | loc:{location_grp} | {days_since}d OVERDUE")
                else:
                    not_submitted_list.append({**base_record, "latestVcrDate": None, "daysSince": None, "status": "Missing"})
            else:
                print(f"[VCR_DASHBOARD] ✗ {eng_name} | Van: {van_name} | loc:{location_grp} | vehicleId: {vehicle_id} → no VCR")
                not_submitted_list.append({**base_record, "latestVcrDate": None, "daysSince": None, "status": "Missing"})

        total = len(submitted_list) + len(not_submitted_list)
        print(f"[VCR_DASHBOARD] ── Done: {len(submitted_list)} submitted, {len(not_submitted_list)} not submitted (total: {total}) ──")
...
def search_vcr_by_van(van_number_or_reg: str):
    try:
        ...
        # Pull photos from Firebase Storage (photos uploaded via the VCR form)
        if FIREBASE_AVAILABLE:
            import re as _re

            def _build_firebase_images(fb_vcr: dict, offset: int) -> list:
                out = []
                for idx, url in enumerate(fb_vcr.get("photos") or []):
                    path  = url.split("?")[0]
                    raw   = path.split("/")[-1]
                    clean = _re.sub(r"^[0-9a-f]{32}_", "", raw)
                    ext   = clean.rsplit(".", 1)[-1] if "." in clean else ""
                    title = clean.rsplit(".", 1)[0].replace("_", " ").replace("-", " ").title()
                    out.append({
                        "id":            f"fb_{fb_vcr.get('id', '')}_{offset + idx}",
                        "title":         title or f"Photo {offset + idx + 1}",
                        "fileExtension": ext,
                        "imageUrl":      url,
                    })
                return out
            ...
            try:
                ...
                def _numeric_suffix(s: str) -> str:
                    """'VEH-00397' → '397', '00397' → '397'"""
                    m = _re2.search(r"(\d+)$", s or "")
                    return str(int(m.group(1))) if m else ""

                # Build all identifiers to try, including the bare number (e.g. 397)
                base_identifiers = [
                    van_number_or_reg,
                    vehicle.get("Name"),
                ...
                for b in base_identifiers:
                    num = _numeric_suffix(b or "")
                    if num and num not in identifiers:
                ...
                # Last resort: scan all recent Firebase VCRs with loose matching
                if not firebase_vcr:
                    all_vcrs = get_all_vcrs_from_firebase(days=30)
                    print(f"[Firebase] Scanning {len(all_vcrs)} recent VCRs, tried identifiers: {seen}")
                    for fb in all_vcrs:
                        fb_van = (fb.get("van_number") or "").strip()
                        fb_van_num = _numeric_suffix(fb_van)
                        for i in seen:
                            i_num = _numeric_suffix(i)
                            if (fb_van.upper() == i.strip().upper() or
                                    (i_num and fb_van_num and i_num == fb_van_num)):
                                if fb.get("photos"):
                                    firebase_vcr = fb
                                    break
                        if firebase_vcr:
                ...
                if firebase_vcr:
                    images.extend(_build_firebase_images(firebase_vcr, len(images)))
                    print(f"[Firebase] Found {len(firebase_vcr.get('photos', []))} photo(s) for {van_number_or_reg}")
                else:
                    print(f"[Firebase] No photos found for {van_number_or_reg} (tried: {seen})")
...
def debug_firebase_vcrs():
    """Shows all recent Firebase VCRs so you can verify what van_number is stored."""
        ...
        return [
            {
                ...
                "sample_photo": (v.get("photos") or [None])[0],
            }
            for v in vcrs
        ]
    except Exception as e:
...
async def proxy_image(version_id: str):
    ...
    try:
        ...
        response = await client.get(rest_url, headers={"Authorization": f"Bearer {access_token}"}, timeout=30.0)
        ...
        return Response(content=response.content, media_type=content_type,
            headers={"Cache-Control": "public, max-age=86400", "Access-Control-Allow-Origin": "*", "Content-Length": str(len(response.content))})
...
def get_single_form_with_images(form_id: str):
    ...
    if doc_link_result:
        ...
        for img in sf_service.execute_soql(f"""
            ...
            WHERE ContentDocumentId IN ('{doc_ids_str}') AND IsLatest = true
        """):
            images.append({"id": img["Id"], "title": img.get("Title", "Image"), "url": f"/api/vehicle-condition/image/{img['Id']}"})
...
async def ai_analyse_vcr_images(form_id: str):
    try:
        ...
        """)
        if not doc_links:
            return {"form_id": form_id, "vehicle": vehicle_name, "reg_no": reg_no, "engineer": engineer_name,
                    "analysed_at": datetime.now(timezone.utc).date().isoformat(), "total_images": 0,
                    "overall_fleet_status": "AMBER", "reports": [], "message": "No images found"}
        ...
        mime_map     = {"jpg": "image/jpeg", "jpeg": "image/jpeg", "png": "image/png", "gif": "image/gif", "webp": "image/webp", "heic": "image/heic"}
        images_for_ai = []
        ...
        for img in image_records:
            ...
            try:
                ...
                if response.status_code != 200 or len(response.content) == 0:
                    continue
                images_for_ai.append({"title": image_title, "base64": base64.b64encode(response.content).decode("utf-8"), "content_type": content_type})
            except Exception as e:
        ...
        reports        = groq_service.analyse_vehicle_images(images=images_for_ai, engineer_name=engineer_name, vehicle_name=vehicle_name, reg_no=reg_no)
        ...
        return {"form_id": form_id, "vehicle": vehicle_name, "reg_no": reg_no, "engineer": engineer_name,
                "analysed_at": datetime.now(timezone.utc).date().isoformat(), "total_images": len(images_for_ai),
                "overall_fleet_status": overall_status, "reports": reports}
    except HTTPException:
        raise
    except Exception as e:
...
def get_vehicle_condition_dashboard():
    try:
        total_vcr_result   = sf_service.execute_soql("SELECT COUNT(Id) cnt FROM Vehicle_Condition_Form__c WHERE CreatedDate = LAST_N_DAYS:14")
        total_vcr          = total_vcr_result[0]['cnt'] if total_vcr_result else 0
        with_images_result = sf_service.execute_soql("SELECT COUNT_DISTINCT(LinkedEntityId) cnt FROM ContentDocumentLink WHERE LinkedEntityId IN (SELECT Id FROM Vehicle_Condition_Form__c WHERE CreatedDate = LAST_N_DAYS:14)")
        with_images        = with_images_result[0]['cnt'] if with_images_result else 0
        without_images     = total_vcr - with_images

        all_vcr          = sf_service.execute_soql("SELECT Id, Name, Vehicle__r.Name, Current_Engineer_Assigned_to_Vehicle__r.Name FROM Vehicle_Condition_Form__c WHERE CreatedDate = LAST_N_DAYS:14 ORDER BY CreatedDate DESC") or []
        image_ids_result = sf_service.execute_soql("SELECT LinkedEntityId FROM ContentDocumentLink WHERE LinkedEntityId IN (SELECT Id FROM Vehicle_Condition_Form__c WHERE CreatedDate = LAST_N_DAYS:14)") or []
        image_id_set     = {rec['LinkedEntityId'] for rec in image_ids_result}

        with_image_list, without_image_list = [], []
        for vcr in all_vcr:
            rd = {"form_id": vcr['Id'], "form_name": vcr['Name'],
        ...
        return {"summary": {"total_vcr": total_vcr, "with_images": with_images, "without_images": without_images, "period": "Last 14 Days"},
